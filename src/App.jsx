  ...
  const RewardsView = () => {
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        {/* WALLET CARD - FIXED VISIBILITY */}
        <div className="p-8 rounded-[44px] text-white shadow-2xl relative overflow-hidden flex justify-between items-center" style={{ backgroundColor: colors.forest }}>
            <div className="relative z-10">
                <p className="text-xs font-bold uppercase tracking-widest opacity-60 mb-2">Kita Points Balance</p>
                <div className="flex items-center gap-4">
                    <h3 className="text-5xl font-black tracking-tight">{kitaPoints.toLocaleString()}</h3>
                    <Wallet size={40} style={{ color: colors.mint }} className="opacity-80" />
                </div>
            </div>
            <div className="absolute -top-10 -right-10 p-4 opacity-10"><Award size={180} /></div>
        </div>

        {/* AVAILABLE REWARDS */}
        <div className="space-y-4">
            <h3 className="text-[10px] font-black uppercase tracking-widest px-3 text-gray-400">Available For You</h3>
            {available.map(item => (
              <div key={item.id} className="bg-white p-5 rounded-[36px] border border-gray-100 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all">
                <div className="flex items-center gap-4">
                    <div className="p-3 bg-gray-50 rounded-2xl text-forest shadow-inner" style={{ color: colors.forest }}>{item.icon}</div>
                    <div><p className="text-sm font-black mb-1" style={{ color: colors.forest }}>{item.partner}</p><p className="text-[11px] font-medium text-gray-400">{item.item}</p></div>
                </div>
                <div className="px-4 py-2 rounded-2xl text-xs font-black shadow-sm" style={{ backgroundColor: colors.mint, color: colors.forest }}>{item.cost} Pts</div>
              </div>
            ))}
        </div>

        {/* REWARD SEPARATOR & HISTORY */}
        {history.length > 0 && (
