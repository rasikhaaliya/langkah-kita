...
const App = () => {
  ...
  const handleJournalSubmit = () => {
    ...
    const currentData = updatedData[activeDate] || { steps: 0, activities: [], journal: "" };
    
    updatedData[activeDate] = {
      ...currentData,
      activities: Array.from(new Set([...currentData.activities, 'Mental Log'])),
  ...
  const OpeningScreen = () => (
    <div className="min-h-screen flex flex-col justify-between p-10 bg-white animate-in fade-in duration-700">
      <div className="mt-16 space-y-6">
        <div className="w-20 h-20 rounded-[28px] flex items-center justify-center shadow-2xl rotate-3" style={{ backgroundColor: colors.forest }}>
          <Footprints size={40} style={{ color: colors.mint }} />
        </div>
        <div className="space-y-2">
            <h1 className="text-6xl font-black tracking-tighter leading-none" style={{ color: colors.forest }}>
            Langkah<br />Kita.
            </h1>
            <div className="h-2 w-16 rounded-full" style={{ backgroundColor: colors.mint }} />
        </div>
      ...
      <div className="space-y-5">
        <button onClick={() => setView('auth')} className="w-full py-5 rounded-[32px] text-white font-bold text-lg shadow-xl flex items-center justify-center gap-2 group active:scale-95 transition-all" style={{ backgroundColor: colors.forest }}>
  ...
  const AuthScreen = () => (
    <div className="min-h-screen flex flex-col p-8 bg-white animate-in slide-in-from-right-5">
      <button onClick={() => setView('opening')} className="p-3 w-fit rounded-2xl bg-gray-50 mb-10 active:scale-90 transition-all" style={{ color: colors.forest }}><ArrowLeft size={20} /></button>
      <div className="mb-10">
      ...
      <form onSubmit={handleAuthSubmit} className="space-y-5">
        {authMode === 'signup' && (
          <div className="space-y-1.5">
            ...
            <input type="text" defaultValue="Athhar" className="w-full py-4 px-6 rounded-[24px] bg-gray-50 border-2 border-transparent focus:border-forest outline-none transition-all font-medium" />
          </div>
        )}
        <div className="space-y-1.5">
          <label className="text-[10px] font-black uppercase tracking-widest ml-4 text-gray-400">Email Address</label>
          <input type="email" defaultValue="athhar@langkahkita.com" className="w-full py-4 px-6 rounded-[24px] bg-gray-50 border-2 border-transparent focus:border-forest outline-none transition-all font-medium" />
        </div>
        <div className="space-y-1.5">
          <label className="text-[10px] font-black uppercase tracking-widest ml-4 text-gray-400">Password</label>
          <input type="password" defaultValue="password123" className="w-full py-4 px-6 rounded-[24px] bg-gray-50 border-2 border-transparent focus:border-forest outline-none transition-all font-medium" />
        </div>
  ...
  const RewardsView = () => (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <h2 className="text-2xl font-black" style={{ color: colors.forest }}>Rewards Hub</h2>
      <div className="p-7 rounded-[40px] text-white shadow-xl relative overflow-hidden bg-forest">
          <div className="relative z-10">
              <p className="text-xs font-bold uppercase tracking-widest opacity-60 mb-1">Total Balance</p>
              <div className="flex items-center gap-3"><h3 className="text-4xl font-black tracking-tighter">{kitaPoints.toLocaleString()}</h3><Wallet size={28} style={{ color: colors.mint }} /></div>
          </div>
          <div className="absolute top-0 right-0 p-4 opacity-10 rotate-12"><Award size={100} /></div>
      </div>
      <div className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-widest px-2 text-gray-400">Available For You</h3>
          {rewardsList.filter(r => !r.redeemed).map(item => (
            <div key={item.id} className="bg-white p-4 rounded-[28px] border border-gray-100 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all">
              <div className="flex items-center gap-4">
                  <div className="p-2.5 bg-gray-50 rounded-2xl shadow-inner" style={{ color: colors.forest }}>{item.icon}</div>
                  <div><p className="text-xs font-black mb-0.5" style={{ color: colors.forest }}>{item.partner}</p><p className="text-[10px] font-medium text-gray-400">{item.item}</p></div>
              </div>
              <div className="px-3.5 py-1.5 rounded-xl text-[10px] font-black shadow-sm" style={{ backgroundColor: colors.mint, color: colors.forest }}>{item.cost} Pts</div>
            </div>
          ))}
      </div>
    </div>
